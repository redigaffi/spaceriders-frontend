  ...
  <div class="row q-ml-xl">
    <div class="col-2 q-mr-lg glass-element-darker" style="border-radius: 0px; height: 650px" >
      ...
      <div class="row q-mt-xs" style="height: 240px; overflow: hidden">
        <div class="col">
          <transition-group name="bounce">
            <div v-for="order in orderBook['sell']" :key="order.grouped_price" class="row full-width q-py-xs"
              style="height: 30px;" :style="{
                background: `linear-gradient(to left, rgba(247, 21, 21, 0.15) ${order['amount_width']}%, transparent 0)`,
              }">
              <div class="col text-left price-text-negative">{{ order.grouped_price }}</div>
      ...
      <div class="row" style="margin-top: 6px; height: 240px; overflow: hidden">
        ...
        <div class="col">
          <transition-group name="bounce">
            <div v-for="order in orderBook['buy']" :key="order.grouped_price" class="row full-width q-py-xs"
              style="height: 30px" :style="{
                background: `linear-gradient(to left, rgba(5, 245, 49, 0.15) ${order['amount_width']}%, transparent 0)`,
              }">
              <div class="col text-left price-text-positive">{{ order.grouped_price }}</div>
              <div class="col text-center">{{ order.sum_to_be_filled }}</div>
              <div class="col text-right">{{ order.total_price }}</div>
            </div>
          </transition-group>
    ...
    <div class="col-6">
      ...
      <div class="row q-mt-sm" style="height: 350px;">
        <q-card style="width: 100%;">
          <q-tabs v-model="userActionTab" inline-label style="height: 47px; width: 100%;"
            class="bg-dark text-white shadow-2">
            <q-tab name="spotTrading" icon="fas fa-exchange-alt" label="Trading" />
            <q-tab name="openTrades" icon="fa fa-book-open" label="Open Trades" />
            <q-tab name="closedTrades" icon="fa fa-book" label="Closed Trades" />
          </q-tabs>
              ...
              <div class="row full-width q-mt-xs">
                <div class="col-6 flex flex-center">
                  <div class="justify-center centers" style="width: 90%;">
                    <q-btn size="sm" :color="tradeType == 'limit' ? 'blue' : 'black'" label="Limit"
                      @click="tradeType = 'limit'" />
                    <q-btn size="sm" class="q-ml-xs" :color="tradeType == 'market' ? 'blue' : 'black'" label="Market"
                      @click="tradeType = 'market'" />
                  </div>
                </div>
              </div>

              <div class="row full-width justify-center centers" style="margin-top: 20px;">
                ...
                <div class="col-6 flex flex-center">
                  <div class="justify-center centers" style="width: 90%;">
                    ...
                    <q-btn @click="sellSpot" color="red" class="full-width q-mt-sm" label="Sell" />
...
function parseCandleData(candle) {
  ...
  return {
    open: candle.open, high: candle.high, low: candle.low, close: candle.close, time: date
  }
}
