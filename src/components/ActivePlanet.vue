<template>
  <q-tabs indicator-color="transparent">
      <q-btn-dropdown auto-close stretch flat  dropdown-icon="far fa-caret-square-down">
          ...
          <q-list>
              ...
              <q-item v-for="p in planets" :key="p.id" clickable>
                <q-item-section align="left" @click="changeActivePlanet(p)">{{ p.name }}</q-item-section>
                <q-icon v-if="isActivePlanet(p)" color="positive" style="margin-top:10px;" name="fas fa-circle" />
              </q-item>
